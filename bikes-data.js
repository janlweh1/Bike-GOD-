...
// Get bikes object from the shared database
function getBikesObject() {
    if (typeof initializeBikes === 'function') {
        initializeBikes();
        return window.bikesDatabase;
    }
    return bikes;
}
...
function renderBikes() {
    ...
    const bikeData = getBikesObject();
    let html = '';
    
    for (const [category, bikeList] of Object.entries(bikeData)) {
        bikeList.forEach(bike => {
            // Skip bikes that have status and are not available
            if (!bike.status || bike.status === 'available') {
...
// Open rental modal - UPDATED to support multiple rentals
function openRentalModal(bikeName, category, price, image) {
    // NO LONGER CHECK FOR EXISTING RENTAL - Allow multiple rentals
...
// Confirm rental and add to active rentals array - UPDATED
function confirmRental() {
    ...
    // Get existing active rentals or initialize empty array
    let activeRentals = [];
    ...
    // Create rental object with unique ID
    const rentalData = {
        ...
        cost: selectedBike.price * duration
    };

    // Add to active rentals array
    activeRentals.push(rentalData);
    ...
    setTimeout(() => {
        window.location.href = 'my_rental.html';
    }, 500);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Confirm button
    document.getElementById('confirmBtn').addEventListener('click', confirmRental);
});