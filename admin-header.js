...
(function () {
        ...
        items.innerHTML = latest.map(n => `
            <div style="padding:10px 12px;border-bottom:1px solid #f1f5f9;display:flex;gap:10px;align-items:flex-start;">
                ...
                <div>
                    ...
                    <div style="font-size:11px;color:#6b7280;margin-top:4px;">${escapeHtml(n.time || formatRelativeTime(n.ts || n.timestamp || 0))}</div>
                </div>
            </div>
        `).join('');
    }
    ...
    function pushNotification(text) {
        ...
        notifQueue.unshift({ text, ts: nowTs });
    ...
    async function checkNotifications() {
        try {
            ...
            if (pData && pData.success && Array.isArray(pData.payments)) {
                ...
                // If there are no in-memory notifications yet, seed from latest activity (up to 8)
