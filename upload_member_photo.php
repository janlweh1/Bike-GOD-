...
if ($conn) {
    ...
    $sql = "EXEC dbo.sp_UpdateMemberPhotoUrl @MemberID = ?, @PhotoUrl = ?";
    $stmt = sqlsrv_query($conn, $sql, [$memberId, $url]);
    if ($stmt === false) {
        $err = sqlsrv_errors();
        // Surface a clear message if the procedure is missing or fails
        $msg = 'Failed to save photo in profile';
        if ($err && isset($err[0]['message']) && stripos($err[0]['message'], 'sp_UpdateMemberPhotoUrl') !== false) {
            $msg = 'Photo procedure sp_UpdateMemberPhotoUrl is missing in the database';
        }
        closeConnection($conn);
        echo json_encode(['success' => false, 'error' => 'db_update_failed', 'message' => $msg]);
        exit();
    }
    closeConnection($conn);
}
